...
def process_one_model(obj_path, n_views):  
    ...
    for i, cam_pos in enumerate(cam_positions):  
        try:
            ...
            # Генерируем случайный UP вектор для этого направления  
            ...
            # Рендерим чистое изображение и получаем данные  
            ...
            # Создаем и сохраняем отладочное изображение  
...
def project_points_to_image(scene, points_3d):  
    ...
    points_2d[:, 1] = (1.0 - (points_ndc_screen[:, 1] + 1.0) * 0.5) * height # Y инвертирован

    # Вычисляем bounding box из видимых точек  
